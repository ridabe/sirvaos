...
- `[ ]` B5 — Template + automação de **confirmação de escala** (usa `send-whatsapp`, context `worship_confirmation`).
- `[ ]` B6 — Template + **lembrete 24h** (job agendado, context `worship_reminder`/`event_reminder`).
...
- `[ ]` B8 — Painel de logs (`whatsapp_messages`) por tenant.
